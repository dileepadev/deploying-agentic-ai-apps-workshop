...
3. You used the `anon` key instead of `service_role`. The anon key is
   blocked by the RLS you enabled — which is exactly its job.
4. The project is **paused**.
...
database looks identical to a broken app.
